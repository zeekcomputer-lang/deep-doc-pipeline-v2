...
**규모:** 12 Python 파일 / 2,826줄 (src 2,388 + scripts 316 + main 122). AST 전 파일 통과.
...
| 버전 | 커밋 | 핵심 변경 |
|------|------|----------|
| **v2.0** | `2b582cb` | 번역 단계 전면 개선: 문단별 분할 번역 + 완전성 검증 + 소스 데이터 폴백 |
...
└── src/
    ├── prompt_config.py     ★ 사용자 커스텀 프롬프트 설정 (목적/톤/독자/지시)
...
TONE_DIRECTIVE = "긴정적 성과와 성장세를 강조하되, 사실에 기반할 것"
...
| 렌더링 톤 불일치 | `_build_faithful_translate_prompt()` 스타일 가이드 조정 |
