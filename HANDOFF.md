...
```
JSONL → 영어로 추출·분석·집필 → 영문 백서 → 한국어 렌더링 → 최종 한글 백서 + DOCX
```

**규모:** 14 파일 / 2,662줄 (src 2,224 + scripts 316 + main 122). AST 전 파일 통과.
...
**유지되는 검증:** section별 `fact_checker` (원본 데이터 대조), `extract_proper_nouns` (렌더링 프롬프트에 고유명사 주입).
...
└── src/
    ...
    ├── context_guard.py     95KB 예산 관리 (172줄)
    ├── llm.py               OpenAI SDK + Rate Limiter + 504 방어 (468줄)
    ├── logger.py            타임라인 로거 (78줄)
    ├── utils.py             Pure Python 결정론 로직 (265줄)
    ├── nodes.py             17개 노드 + 3개 라우터 (1,017줄)
...
START → load_docs → [fanout] strict_extractor(×N) → chrono_sorter     Phase 1: 추출
     ...
     → prepare_translation → translate → END                          Phase 5: 렌더링
```
...
| 5 | 고유명사 보존 | `extract_proper_nouns` → 렌더링 프롬프트에 목록 주입 |
...
5. 영어 출력 노드는 `_EN_ENFORCE` 필수, 렌더링 노드는 미사용
...
# 백서 생성
python -m main                         # reasoning=high (기본)
...
### C: "렌더링 스타일 변경"
→ `src/nodes.py` `_build_render_prompt()` 수정
...
| 렌더링 톤 불일치 | `_build_render_prompt()` 스타일 가이드 조정 |
