...
저성능 LLM(gpt-oss) 환경에서 **환각을 구조적으로 차단**하며 JSONL 원시 문서 → **카테고리 기반 지식 구조화** → **경영진용 한글 백서**를 자동 생성하는 **LangGraph 파이프라인**.

v2는 시간순 우선(Phase 1-5)이었다. v3는 **카테고리 우선 4-Step** 아키텍처:

```
JSONL → 4-카테고리 지식 분류 → 카테고리별 심층 분석 → 경영 요약서 집필
     → 시간순 부록 포맷팅 → 본문+부록 조립 → 윤문 → (선택) 한국어 번역
```

**핵심 전환:** 시계열 종속 탈피. 날짜 마커 없는 데이터도 처리 가능.
...
| 버전 | 커밋 | 핵심 변경 |
|------|------|----------|
| **v3.0** | (구현 대기) | 카테고리 우선 지식 구조화, 4-Step 하이브리드 백서, 날짜 무관 처리 |
| v2.0 | `09fda16` | 번역 v2 + prompt_config 커스텀 + --skip-fact-check |
...
2-3페이지 고압축 비즈니스 백서. 집필 루프: `section_writer` ⟲ `fact_checker` (선택).
v2 방어 기제 동일: `previous_draft` + `hallucinated_tokens` 블랙리스트, 3회 실패 시 ⚠️ 워터마크.
산출물: `step3_executive_summary.md`

### Step 4 — 하이브리드 조립 (Hybrid Assembly)

시간순 인덱스 → 부록 포맷팅 → 경영 요약서(본문) + 부록 조립 → 윤문 → (선택) EN→KR 번역.
산출물: `step4_appendix_timeline.md` + `step4_final_en.md` + `step4_final_kr.md`
...
START → load_docs → [fanout] knowledge_extractor(×N) → knowledge_aggregator → temporal_indexer
     ...
     → init_writing → section_writer ⟲ fact_checker → ...save loop...
     → timeline_formatter → compiler → polish
...
| 3 | `fact_checker` | LLM (선택) | 초안 vs 지식 베이스 대조 |
| 3 | `retry_section` | Python | 반려 시 재작성 트리거 |
| 3 | `save_section` | Python | 승인 섹션 저장 |
| 3 | `save_section_with_warning` | Python | ⚠️ 강제 저장 + 감사 로그 |
| 4 | `timeline_formatter` | LLM | 시간순 부록 마크다운 생성 |
| 4 | `compiler` | Python | 본문+부록 조립 (LLM 금지) |
| 4 | `polish` | LLM | 최종 윤문 |
| 4 | `prepare_translation` | Python | 영문 저장 + 고유명사 추출 |
| 4 | `translate` | LLM | 섹션별 문단 분할 번역 |

**라우터:** `fanout_to_extractor` · `fanout_to_category_analyzer` · `route_narrative` · `route_section_draft` · `route_next_section`
...
output/<timestamp>/
  ...
  ├── step4_final_en.md               최종 영문 백서
  └── step4_final_kr.md               최종 한글 백서 (번역 시)
```
...
| 3 | 504 타임아웃 | 국부 감축 (-5KB/step) + reasoning 다운그레이드 | `llm.py` |
| 4 | 95KB 초과 | `effective_budget()` 사전 측정 + 분할/압축 | `context_guard.py` |
| 5 | 고유명사 소실 | `extract_proper_nouns` → 번역 프롬프트 주입 | `utils.py` / `translate` |
| 6 | 날짜 유연 처리 | `temporal_indexer` graceful — `null` 날짜 허용 | `temporal_indexer` |
| 7 | 카테고리 불균형 | `knowledge_aggregator` 경고 로그 + 비율 보고 | `knowledge_aggregator` |
| 8 | 지식 베이스 내보내기 | `--export-kb` 독립 JSON 추출 | `main.py` |
...
5. **EN-only LLM (Step 1-3)** — 한국어는 Step 4 번역에서만.
...
| 설정 | Step 1 (extractor) | Step 2 (analyzer, planner) | Step 3 (writer) | Step 4 (formatter) | 번역 |
|------|:---:|:---:|:---:|:---:|:---:|
| `DOCUMENT_PURPOSE` | ✓ | ✓ | ✓ | ✓ | ✓ |
| `TONE_DIRECTIVE` | — | ✓ | ✓ | — | ✓ |
| `TARGET_AUDIENCE` | — | — | ✓ | ✓ | ✓ |
...
⚠️ 편향 설정과 무관하게 `fact_checker`가 지식 베이스 외 사실 추가를 차단.
...
python -m main                            # 기본 (팩트체크 ON, 번역 ON)
python -m main --skip-fact-check          # 팩트체크 생략
python -m main --skip-translation         # 번역 생략 (영문만)
...
| `EXTRACTOR_MODEL` / `JUDGE_MODEL` / `WRITER_MODEL` | (OPENAI_MODEL) | 역할별 분리 |
