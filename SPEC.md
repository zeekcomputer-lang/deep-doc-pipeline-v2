...
**유지 항목:** 순수 OpenAI SDK, Pydantic 강제 출력, 95KB 하드리밋, 504 국부 감축, user 메시지 불변, 팩트체커 루프, `prompt_config.py` 커스텀.
...
- `fact_checker`에서 고유명사 번역/변형을 환각으로 판정하지 않도록 예외 처리
...
| 6 | 날짜 부재 | `temporal_indexer` best-effort: 정규식 폴백 → `"undated"` 그룹 | `temporal_indexer` |
...
```bash
python -m main                                        # 기본 (reasoning=high, 팩트체크 ON, 한국어 직접 출력)
