...
| # | 원칙 | 설명 |
|---|------|------|
| 1 | **카테고리 우선 지식 구조화** | 원시 데이터를 시간이 아닌 비즈니스 의미 축으로 먼저 분류. **카테고리는 '지식 분류 축'일 뿐, 문서 섹션과 1:1 대응하지 않음** (§1.3 참조). 날짜 없는 데이터도 처리 가능. |
...
### 1.3 카테고리·섹션 분리 전략 (v3.1)

> **핵심:** 카테고리는 지식을 정리하는 **분류 축(index)**일 뿐, 최종 문서의 섹션 구조와 1:1로 대응하지 않는다.

- **문제:** narrative_planner가 4개 카테고리 분석을 그대로 받아 카테고리당 섹션 1개를 만들면, 문서 단락이 카테고리와 동일 수준으로 고정된다.
...
> **시점 참고 정보 제공 (Temporal Reference, v3.1):** `INCLUDE_TEMPORAL_CONTEXT=True`(기본)이면 `_build_temporal_block()`이 분석·집필 노드에 "날짜를 **참고 정보**로 제공하며, 서술에 도움이 되는 경우에만 시점을 포함하라(강제 아님, 선택 사항)"는 안내를 주입한다. 데이터는 `format_entries_for_prompt`가 `[YYYY-MM-DD]`로 이미 전달. 시점 포함은 필수가 아니며 비즈니스 인사이트 전달이 우선. 날짜 없는 항목은 시점을 지어내지 않아 환각 방지. 월별 상세 타임라인 '부록'과는 별개(부록은 제거됨).
