...
**해결 (초기):** `prompt_config.INCLUDE_TEMPORAL_CONTEXT`(기본 True) 추가. True면 `_build_temporal_block()`이 분석·집필 컨텍스트에 시점 안내 주입.

**보정 (톤 약화):** 이후 사용자가 "시간이 단서로 있어도 반드시 포함될 필요는 없다, 단순 참조 정보로만 제공"을 요청. 지시문을 "시점 서술 규칙(포함하십시오)" → "시점 참고 정보(포함할 수 있습니다, 필수 아님, 생략 가능)"로 완화. 시점 표기는 선택 사항이고 인사이트 전달이 우선. 날짜 없는 사안은 그대로 환각 방지.

**원칙:**
> LLM에게 정보를 "보여주는 것"과 "활용하라고 지시하는 것"은 다르다. 그리고 "활용"의 강도도 조절 대상이다 — 필수(강제) vs 참고(선택). 요구가 '단순 참조'면 지시문을 선택형으로 쓴다.

**적용:** `prompt_config`(INCLUDE_TEMPORAL_CONTEXT/_TEMPORAL_DIRECTIVE — 참고 정보 톤/_build_temporal_block/get_temporal_directive, analysis·writing context), `utils.format_entries_for_prompt(show_date)`
...
## L-027: 섹션별 윤문(polish) 시 헤딩 소실 — 헤딩은 LLM에 보내지 마라

**증상:** step4 최종 문서에서 일부 단락의 제목(예: '## 시사점 및 제언')이 소실. 본문은 그대로 있음.

**근본 원인:** `polish_node`가 504 방지를 위해 `split_by_section`으로 `## ` 단위로 나눈 뒤 **각 섹션을 '## 제목 + 본문' 통째로 LLM에 전달**해 윤문했다. LLM이 윤문 응답에서 헤딩 라인을 누락시키면 제목이 사라진다. 프롬프트로 '헤딩 유지'를 요청해도 저성능 모델은 종종 누락.

**해결:** 윤문 단계에서 헤딩을 LLM에 아예 보내지 않는다. `split_heading_body()`로 (헤딩, 본문) 분리 → 본문만 LLM 윤문 → 헤딩은 결정론적으로 재결합. LLM이 제목을 건드릴 수 자체를 없앨.
...
_본 문서는 본 프로젝트뿐 아니라 향후 LangGraph/LLM 파이프라인 작업에 참고 가능._
