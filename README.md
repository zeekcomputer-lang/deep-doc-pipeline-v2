...
JSONL 원본 데이터를 **카테고리별 지식 구조화** → **서사 설계** → **팩트 체킹** → **최종 백서**로 변환합니다.
...
```bash
python -m main --skip-fact-check          # 팩트 체킹 생략 (빠른 실행)
python -m main --skip-translation         # 번역 단계 생략
...
load_docs → knowledge_extractor(×N) → knowledge_aggregator → temporal_indexer
         ...
         → section_writer ⟲ fact_checker → …
         → timeline_formatter → compiler → polish → (translate) → END
```
