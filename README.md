...
### 문서 커스터마이징 (`src/prompt_config.py`)

```python
DOCUMENT_TYPE = "whitepaper"       # ★ 문서 유형 선택
DOCUMENT_TITLE = ""                # 표지 제목 (비우면 LLM 자동 생성)
DOCUMENT_PURPOSE = ""              # 문서 목적 (비우면 유형별 기본값)
DOMAIN_KNOWLEDGE = """            # ★ LLM이 모르는 사전 지식 주입
- 개발 단계: 기획 → 설계 → 구현 → 안정화 → 운영전환
- 'P99'은 상위 1% 느린 요청의 응답시간을 의미한다
"""
KEY_TERMS = {"Go-Live": "무중단 운영 전환 단계"}   # 용어집
INCLUDE_TEMPORAL_CONTEXT = True    # 날짜를 참고 정보로 제공, 서술에 도움 될 때만 반영 (강제 아님, 기본 True)
```

> **시점 참고 정보:** `INCLUDE_TEMPORAL_CONTEXT=True`이면 데이터의 날짜 단서(`date_hint`)를 **참고 정보**로 제공하고, 서술의 자연스러움이나 선후 관계에 도움이 되는 경우에만 시점을 반영합니다(예: '2026년 2월'). **시점 포함은 강제가 아니며**(선택 사항), 비즈니스 인사이트 전달이 우선입니다. 날짜 없는 사안은 지어내지 않아 환각을 방지합니다. (월별 상세 타임라인 '부록'과는 별개 — 부록은 제거됨.)

### 문서 유형 (Document Type) — 카테고리·섹션 분리 전략

카테고리(4축)는 **지식 분류 축**일 뿐, 문서의 섹션 구조와 1:1로 대응하지 않습니다. 동일한 지식 베이스에서 `DOCUMENT_TYPE`만 바꿔 용도에 맞는 다양한 문서를 생성합니다.

| 유형 | 산출물 |
|------|--------|
| `whitepaper` | 경영진 비즈니스 백서 (기본). 문제→결정→가치→교훈 서사 |
| `executive_brief` | 1페이지 임원 요약보고서 (핵심 압축) |
| `postmortem` | 장애·회고 보고서 (상황·원인·조치·재발방지) |
| `tech_report` | 기술 현황 보고서 (아키텍처·의사결정·리스크) |
| `status_update` | 이해관계자 진행 업데이트 (성과·이슈·다음 단계) |
| `custom` | `DOCUMENT_TYPE_CUSTOM_STRUCTURE`에 직접 구조 지정 |

```python
DOCUMENT_TYPE = "postmortem"   # 같은 지식 → 장애 회고 보고서로 생성
# 또는
DOCUMENT_TYPE = "custom"
DOCUMENT_TYPE_CUSTOM_STRUCTURE = "1) 배경 2) 핵심 제안 3) 기대효과 구조로 작성"
```

> **동작 원리:** `narrative_planner`가 카테고리별 지식을 '소재'로 받아, 문서 유형에 맞는 '주제' 중심 섹션을 설계합니다. 하나의 섹션이 여러 카테고리를 가로지르고(cross-cutting), 카테고리명을 그대로 섹션 제목으로 쓰지 않습니다.

> **능동적 문단 설계:** 섹션 수·깊이는 고정되지 않으며, LLM이 내용의 풍부함과 문서 유형에 맞게 스스로 결정합니다. 내용이 많은 섹션은 `### 소제목`(subsections)으로 세분되고, 단순한 섹션은 한 문단으로 유지됩니다. 권장 섹션 수는 참고일 뿐 강제가 아닙니다.
